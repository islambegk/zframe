...
(defn fetch [opts cb]
  #?(:cljs
     (-> (js/fetch (str (aget js/window "api-base-url") "/rpc?_format=transit")
                   (clj->js {:method "post", :headers {"accept" "application/json", "Content-Type" "application/transit+json"},
                             :body (to-transit (select-keys opts [:method :params :id :policy])), :signal (:signal opts)} :keyword-fn (fn [x] (subs (str x) 1))))
         (.then
          (fn [resp]
            (.then (.text resp)
                   (fn [doc]
     ...
     (if (and debounce path (not force))
       ...
       (let [db db/app-db
        ...
        (-> (js/fetch (str (aget js/window "api-base-url")
                           "/rpc?_format=" (or (some-> transport name) "transit")
                           "&_m=" (:method opts))
