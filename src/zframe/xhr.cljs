...
(defn *json-fetch [{:keys [uri format headers is-fetching-path path params success error cache] :as opts}]
  (let [db @db/app-db
        base-url (or (get-in db [:api-base-url])
                     (get-in db [:xhr/config :base-url]))
        token (get-in db [:xhr/config :token])
        ...
        fetch-opts (-> (merge {:method "get" :mode "cors"} opts)
                       ...
                       (update :method (fn [method] (str/upper-case (name  method))))
        ...
        fetch-opts (cond-> fetch-opts
                     ...
                     (contains? #{"GET" "HEAD"} (:method fetch-opts))
                     (dissoc :body))
                       ...
                       (do (rf/dispatch [(or (:event error) :json/fetch-error)
                                         (merge error
                                                {:request opts
                                                 :response resp
                                                 :data doc})])
                           (when (or (nil? (:xhr/push-message opts)) (:xhr/push-message opts))
              ...
              (.then (.json resp)
                     (fn [doc]
                       (let [data (js->clj doc :keywordize-keys true)
                             data (if (:unbundle opts)
                                    (mapv :resource (:entry data))
                                    data)]
                         (if (< (.-status resp) 299)
                           (do
                             (when path (rf/dispatch [::save path {:status "done" :data data}]))
                             (when (:event success)
                               (rf/dispatch [(:event success)
                                             (merge success
                                                    {:request opts
                                                     ...
                                                     :data data})])))
                           (do
                             (when path (rf/dispatch [::save path {:status "error" :error data :http-status {:code (.-status resp) :text (.-statusText resp)}}]))
                             (when (:event error)
